...
def find_project_root(start: Path, marker_folder: str = "assets") -> Path:
    ...
    Se o script foi movido para a raiz, 'start' já é o diretório correto.
    """
    p = start
    # Se o arquivo estiver em '/home/user/repo/Apresentacao.py', parent é '/home/user/repo'
    # Se o arquivo estiver em '/home/user/repo/scripts/Apresentacao.py', parent é '/home/user/repo/scripts'
...
# Nota: Se você moveu Apresentacao.py para a raiz, mude a linha abaixo para:
...
def img_circular_b64(path: Path) -> str:
    ...
    try:
        ...
        # st.error(f"Erro ao processar imagem circular: {e}") # Desativado para não poluir o Streamlit
        return ""

def tag_html_profile(base64_img: str, name: str, caption: str):
    """
    Gera APENAS o bloco HTML do perfil. O CSS está sendo injetado separadamente
    no topo do script.
    """
    html_content = f"""
...
def get_apresentacao_content():
    # ... (O conteúdo da apresentação em Markdown permanece inalterado) ...
    # (Para brevidade, o conteúdo completo foi omitido aqui)
    content = []
...
def generate_docx(content_markdown: str) -> BytesIO:
    """Gera o arquivo DOCX em memória."""
    document = Document()
...
def generate_pdf(content_markdown: str) -> BytesIO:
    """
    Gera o arquivo PDF em memória usando fpdf2.
    Correção de erro de codificação e renderização.
    """
    class PDF(FPDF):
        def header(self):
            self.set_font('Arial', 'B', 15)
            # Codificação para fpdf2
    ...
    for line in content_markdown.split('\n\n'):
        ...
        if line.startswith('#'):
            ...
            if text:
                ...
                # Adicionado altura de linha para evitar o erro de 'Espaço insuficiente'
...
# ---------------------------------
# CORREÇÃO CRÍTICA: INJEÇÃO GLOBAL DO CSS (Resolve o erro de DOM/removeChild)
# ---------------------------------
custom_css = """
<style>
    .profile-container {
...
"""
# Apenas a injeção do CSS (unsafe_allow_html=True) UMA ÚNICA VEZ
...
with col_main:
    ...
    # ---------------------------------
    # -------- Orientando (AGORA SEM INJEÇÃO REPETIDA DE CSS) --------
    # ---------------------------------
    ...
    # ---------------------------------
    # -------- Orientadora (AGORA SEM INJEÇÃO REPETIDA DE CSS) --------
    # ---------------------------------
    ...
    # ---------------------------------
    # SEÇÃO 0: TEMA, DELIMITAÇÃO e PROBLEMA (O conteúdo do restante da página deve ser inserido aqui)
    # ---------------------------------
    ...
    # ... (O restante das seções (1 a 7) e abas devem ser inseridas aqui) ...
    ...
    with col_pdf:
        try:
            ...
            st.download_button(
             ...
             # Este erro só deve aparecer se houver algum caractere extremo no texto
